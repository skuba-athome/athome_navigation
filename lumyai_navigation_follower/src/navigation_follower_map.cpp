...
static geometry_msgs::PoseStamped::Ptr goal2(new geometry_msgs::PoseStamped);
...
void cmdCallback(const lumyai_navigation_msgs::NavGoalMsg::Ptr& new_goal)
{
	...
	goal_temp->pose.position.y = new_goal->pose2d.y;
  	...
  	try{
  		listener->waitForTransform(odom_frame, goal_temp->header.frame_id, goal_temp->header.stamp, ros::Duration(1.0));
  		listener->transformPose(odom_frame,*goal_temp,*goal2);
		pass_goalcb = true;
  	}
  	catch(tf::TransformException& ex){
		ROS_ERROR("Received an exception trying to transform a point from %s to %s: %s", goal_temp->header.frame_id.c_str(),odom_frame.c_str(),ex.what());
  	}
}
...
void trajectory_con()
{
	...
	global_costmap_ros->setConvexPolygonCost(clear_poly, costmap_2d::FREE_SPACE);

	clear_poly.clear();

	x = goal2->pose.position.x;
	y = goal2->pose.position.y;
	...
	local_costmap_ros->setConvexPolygonCost(clear_poly, costmap_2d::FREE_SPACE);
	...
	global_costmap_ros->getRobotPose(robot_pose_tf);
	...
	if(plan_status && tc->setPlan(path))
	{
		plan_status = true;
	}
	else
	...
	if(robot_stop)
	{
		...
		cmd_vel.linear.y = 0.0f;
		cmd_vel.angular.z = 0.0f;
		global_costmap_ros->resetMapOutsideWindow(0,0);
		local_costmap_ros->resetMapOutsideWindow(0,0);
	} 
...
int main(int argc, char** argv)
{
	...
	listener = new tf::TransformListener();
    	...
    	global_costmap_ros = new costmap_2d::Costmap2DROS("global_costmap", *listener);
	global_costmap_ros->pause();
	...
	planner->initialize(bgp_loader.getName("navfn/NavfnROS"), global_costmap_ros);

	local_costmap_ros = new costmap_2d::Costmap2DROS("local_costmap", *listener);
	local_costmap_ros->pause();
	...
	tc->initialize(blp_loader.getName("base_local_planner/TrajectoryPlannerROS"), listener, local_costmap_ros);//dwa_local_planner/DWAPlannerROS
		
	global_costmap_ros->start();
